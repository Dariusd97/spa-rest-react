...
class DeviationDailyList extends Component{
    constructor(props){
        super(props)
        this.state = {
            dailyDeviations : []
    ...
    componentDidMount(){
        this.store.getDevianDaily()
        this.store.emitter.addListener('GET_ALL_SUCCESS',() => {
            this.setState({
                dailyDeviations:this.store.dailyContent
            })
        })
    }
    ...
    render(){
        return(
            <table  style={{backgroundColor:"#F0F8FF",display:'block'}}>
                ...
                {
                    this.state.dailyDeviations.map((e,i) => 
...
export default DeviationDailyList