...
class DeviationNewestList extends Component{
    constructor(){
        super()
        this.state = {
            newestDeviations : []
    ...
    componentDidMount(){
        this.store.getDevianNewest()
        this.store.emitter.addListener('GET_ALL_SUCCESS',() => {
            this.setState({
                newestDeviations:this.store.newestContent
            })
        })
    }
    ...
    render(){
        return(
            <table  style={{backgroundColor:"#F0F8FF",display:'block'}}>
                ...
                {
                    this.state.newestDeviations.map((e,i) => 
...
export default DeviationNewestList