...
class DeviationPopularList extends Component{
    constructor(){
        super()
        this.state = {
            popularDeviations : []
    ...
    componentDidMount(){
        this.store.getDevianPopular()
        this.store.emitter.addListener('GET_ALL_SUCCESS',() => {
            this.setState({
                popularDeviations:this.store.popularContent
            })
        })
    }
    ...
    render(){
        return(
           <table  style={{backgroundColor:"#F0F8FF",display:'block'}}>
                ...
                {
                    this.state.popularDeviations.map((e,i) => 
                    <tr key={i} style={{width:400}}>
                        ...
                        <th>
                            <input type="button" value="add" onClick={() => this.addOne(e.username,e.image)} />
                        </th>
                    </tr>)
                }
                </tbody>
...
export default DeviationPopularList